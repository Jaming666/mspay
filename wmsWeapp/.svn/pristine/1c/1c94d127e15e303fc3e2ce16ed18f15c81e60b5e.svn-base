...
import { createOrderNoout } from "src/api/common";
import { getOrderDetails, getLinkedOrders } from "src/api/navigateToout";
import { getdepartmentout } from "src/api/common";
...
const formData = ref({
  shipmentOrderNo: "",
  bpmNo: currentPage.bpmNo || "",
  /** 出库名称 */
  shipmentOrderName: "维修工单申请领料",
  /** 出库类型 */
  shipmentOrderType: "",
  /** 是否关联订单 */
...
const profitCenterChange = async (e) => {
  ...
  if (value == "G001" || value == "G008") {
    const departmentList = await getdepartmentout({ profitCenterNum: value });
    codesStore.setDepartmentList(departmentList);
    showSecondSelect.value = true;
  } else {
    showSecondSelect.value = false;
...
const getDetails = async (id) => {
  ...
  orderStore.setOrderInfo(formData.value);
  await codesStore.getProfitCenterout({ warehouseId: res.warehouseId });
};
const initView = async () => {
  await orderStore.getCangKuOut(currentPage.usercode);
  if (currentPage.id) {
    getDetails(currentPage.id);
  } else {
    // 创建出库单号
    const noRes = await createOrderNoout("E");
    if (noRes) {
  ...
  if (orderStore.warehouse.length == 1) {
    ...
    // 保存仓库id,后面添加物料查询的时候要用
    orderStore.setOrderInfo(formData.value);
    await codesStore.getProfitCenterout({
...
<template>
  <div class="pd-20">
    ...
    <nut-form ref="formRef" :model-value="formData">
      ...
      <nut-form-item label="工单编号" required :label-width="labelWidth">
        <nut-input
          :disabled="true"
