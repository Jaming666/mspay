...
import { getSupplier } from "../../../api/navigateToout";
...
const getAreaList = async () => {
  const res = await getAreaByWareout(orderStore.orderInfo.warehouseId);
  if (res?.content) {
    areaList.value = res.content;
  }
};
const getSubjectList = async () => {
  const res = await getSubjectout({ level: 1 });
...
const getRemarkEntryList = async () => {
  const res = await getRemarkEntryout();
  if (res?.data) {
...
const getCount = async () => {
  const res = await scanItemQrCodeout({
