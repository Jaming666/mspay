...
import { getAreaByWareout } from "src/api/in";
import { getItems } from "src/api/navigateToout";
import { getItemTypeListout } from "src/api/common";
...
const getAreaList = async () => {
  const res = await getAreaByWareout(formData.value.warehouseId);
  if (res?.content) {
...
const getGoodClass = async (level: number, parentId?: string = "") => {
  const res = await getItemTypeListout({
    level,
    parentId,
  });
  if (res?.content) {
